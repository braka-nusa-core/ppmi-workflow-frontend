...
import { createQS }          from '@/lib/api/qs'
import { resolveDivisionId } from '@/lib/api/divisions'
...
export function QSCreateClient() {
  ...
  // ── Shared mutation ───────────────────────────────────────────
  // Both "Save Draft" and "Submit" call createQS(); they differ only in status.
  const createMutation = useMutation({
    mutationFn: async ({ data, status }: { data: CreateQSFormData; status: 'DRAFT' | 'SUBMITTED' }) => {
      const divisionId = await resolveDivisionId(data.division)
      return createQS({ ...data, status }, divisionId)
    },
    onSuccess: (created) => {
      queryClient.invalidateQueries({ queryKey: ['qs-list'] })
      success('QS Created', `${created.docNumber} has been saved successfully.`)
      router.push('/dashboard/qs')
    },
    onError: () => {
      toastError('Save failed', 'Could not save the QS. Please check your inputs and try again.')
    },
  })

  // Save as Draft — does NOT require form validation (partial save)
  ...
  // Submit for approval — requires full form validation
  const onSubmit = (data: CreateQSFormData) => {
  ...
  const isSaving     = createMutation.isPending && createMutation.variables?.status === 'DRAFT'
  const isSubmitting = createMutation.isPending && createMutation.variables?.status === 'SUBMITTED'
        ...
        <div className="flex-1 overflow-y-auto page-container pt-6">
          ...
          >
            ...
            <p className="text-[12px] text-[#123d6b]">
              QS number will be auto-generated by the server on save.
            </p>
          </div>
